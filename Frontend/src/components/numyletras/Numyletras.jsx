...
function Navbar() {
  ...
  const iniciarGrabacionYReconocimiento = async () => {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      alert(
        "getUserMedia no es compatible con tu navegador. Prueba con Chrome o Firefox."
      );
      return;
    }
    try {
      ...
      recognition.onerror = (event) => {
        ...
        if (event.error === "no-speech") {
          errorMessage =
            "No se detectó voz. Por favor, asegúrate de hablar claro.";
        } else if (event.error === "aborted") {
          errorMessage = "La grabación fue cancelada.";
        } else if (event.error === "audio-capture") {
          errorMessage =
            "Problema con el micrófono. Asegúrate de que esté conectado y no esté en uso por otra aplicación.";
        } else if (event.error === "not-allowed") {
          errorMessage =
            "Permiso de micrófono denegado. Por favor, habilítalo en la configuración de tu navegador.";
        } else {
          errorMessage =
            "Ocurrió un error inesperado en el reconocimiento de voz: " +
            event.error;
        }
        alert(errorMessage);
        stopInitiatedByUser.current = true;
      ...
      recognition.onend = () => {
        console.log(
          "Recognition onend fired. 'grabando' state (before cleanup):",
          grabando
        );
        console.log(
          "stopInitiatedByUser.current:",
          stopInitiatedByUser.current
        );
        console.log(
          "grabAndRecognizeTriggered.current:",
          grabAndRecognizeTriggered.current
        );

        if (
          stopInitiatedByUser.current ||
          (grabando && grabAndRecognizeTriggered.current)
        ) {
          const textToSend = currentRoundFullTextRef.current.trim();
          console.log(
            "ONEND: Texto final para enviar:",
            textToSend,
            "Length:",
            textToSend.length
          );
  ...
  const enviarTranscripcionAlBackend = async (transcripcionTexto) => {
    ...
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/numyletras/procesar_texto/",
        {
          method: "POST",
          body: formData,
        }
      ...
      setEsCorrecto(false);
      setTranscripcion(
        transcripcionTexto ? [transcripcionTexto] : ["Error de red/servidor"]
      );
    } finally {
      setCargando(false);
  ...
  function generarSecuenciaMixta() {
    ...
    while (mezcla.length < 5) {
      ...
      if (usarLetra && posiblesLetras.length > 0) {
        const letra = posiblesLetras.splice(
          Math.floor(Math.random() * posiblesLetras.length),
          1
        )[0];
        mezcla.push(letra);
      } else if (posiblesNumeros.length > 0) {
        const numero = posiblesNumeros.splice(
          Math.floor(Math.random() * posiblesNumeros.length),
          1
        )[0];
        mezcla.push(numero);
      }
  ...
  return (
    <div className="nums-navbar-container">
            ...
            {modoModal === "inicio" ? (
              <>
                ...
                <ol style={{ textAlign: "justify" }}>
                  ...
                  <li>
                    Memoriza u ordena en tu mente la serie de numeros y letras
                    que se te muestran (Recuerda que las letras son según el
                    orden alfabético){" "}
                  </li>
                  <li>
                    PRIMERO DI LOS NÚMEROS Y LUEGO LAS LETRAS cuando comience la
                    grabación.
                  </li>
                  <li>
              ...
              <>
                ...
                <ul style={{ textAlign: "left" }}>
                  <li>
                    {" "}
                    Observa la secuencia de números o letras que aparece por 5
                    segundos.
                  </li>
                  <li>
                    {" "}
                    Cuando empiece la grabación, di la secuencia comenzando con
                    los NÚMEROS, según la instrucción mostrada y da click en
                    detener cuando hayas terminado.
                  </li>
                  <li>
                    {" "}
          ...
          <div className="Numeros">
            {cargando ? (
              ...
              <>
                ...
                <p>
                  {numeros
                    .map((n) => (typeof n === "string" ? n.toUpperCase() : n))
                    .join(" ")}
                </p>
              </>
            ) : mostrarResultado ? (
              <>
                ...
                <h6>
                  ...
                  {transcripcion.join(" ").toUpperCase() || "Sin transcripción"}
                </h6>
                <br></br>
                <h6>
